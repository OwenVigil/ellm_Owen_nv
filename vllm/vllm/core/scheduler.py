...
class SchedulerOutputs:
    ...
    num_batched_tokens: int
    # Prefix tokens regenerated by the static eLLM path.
    num_ellm_recompute_tokens: int
...
class Scheduler:
    ...
    def __init__(
        ...
        block_manager_kwargs = {}
        if self.scheduler_config.ellm_config.enabled:
            block_manager_kwargs["ellm_drop_ratio"] = (
                self.scheduler_config.ellm_config.drop_ratio)
    ...
    def _schedule_running(
        ...
        while running_queue:
            ...
            ellm_config = self.scheduler_config.ellm_config
            group_recompute_tokens = 0
            if ellm_config.enabled and not seq_group.is_prefill():
                seqs = seq_group.get_seqs(status=SequenceStatus.RUNNING)
                if len(seqs) != 1:
                    raise RuntimeError("eLLM currently supports one sequence "
                                       "per sequence group.")
                group_recompute_tokens = sum(
                    self.block_manager.get_num_recompute_tokens(seq)
                    for seq in seqs)
                assert ellm_config.max_recompute_tokens is not None
                if group_recompute_tokens > ellm_config.max_recompute_tokens:
                    raise RuntimeError(
                        "A single eLLM request requires "
                        f"{group_recompute_tokens} recompute tokens, exceeding "
                        "--ellm-max-recompute-tokens="
                        f"{ellm_config.max_recompute_tokens}.")
                if (num_recompute_tokens + group_recompute_tokens >
                        ellm_config.max_recompute_tokens):
                    break

            running_queue.popleft()
            if group_recompute_tokens:
                for seq in seq_group.get_seqs(
                        status=SequenceStatus.RUNNING):
                    self.block_manager.evict_recompute_prefix(seq)
            while not self._can_append_slots(seq_group):
                ...
                num_recompute_tokens += group_recompute_tokens
        ...
        return running_queue, SchedulerRunningOutputs(
            ...
            num_lookahead_slots=self._get_num_lookahead_slots(
                is_prefill=False),
            num_ellm_recompute_tokens=num_recompute_tokens)
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        return SchedulerOutputs(
            ...
            num_ellm_recompute_tokens=(
                running_scheduled.num_ellm_recompute_tokens),
